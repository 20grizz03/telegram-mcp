...
| `search_channels` | Глобальный поиск публичных Telegram-каналов, включая каналы без подписки |
| `search_public_posts` | Глобальный поиск публикаций в публичных каналах с проверкой бесплатной квоты и запретом списания Stars |
...
`search_public_posts` может использовать один из бесплатных дневных запросов,
выдаваемых Telegram. Перед поиском сервер проверяет квоту и не выполняет запрос,
если он требует оплаты. Поле `allow_paid_stars` не передаётся ни при каких
условиях, поэтому инструмент не может списать Stars.
...
| `publish_rich_post` | Нативный Rich Text: заголовки, таблицы, списки, цитаты, код, формулы и inline-медиа |
| `edit_post` | Изменение текста опубликованного поста |
| `edit_rich_post` | Изменение нативного Rich Text-поста |
| `edit_scheduled_post` | Изменение текста или подписи ожидающего отложенного поста |
| `delete_scheduled_post` | Удаление ожидающего поста из очереди публикаций |
...
`publish_rich_post` и `edit_rich_post` используют нативный Rich Text Telegram
(MTProto layer 227) и требуют Premium у авторизованного аккаунта. Передайте
ровно одно поле: `markdown` или `html`. Поддерживаются заголовки, таблицы,
списки, цитаты, блоки кода, формулы, details и медиа между абзацами. Внешнее
медиа можно вставлять обычным HTTP(S)-URL. Локальное фото передаётся в `photos`
с коротким `id`, а в тексте размещается как `![](tg://photo?id=cover)`.

Пример Rich Markdown:

```markdown
# Короткий разбор релиза

| Что изменилось | Практический эффект |
| --- | --- |
| Новый API | Меньше ручного кода |

> Ограничение: функция пока доступна только Premium.

![](tg://photo?id=cover "Официальная схема")
```
...
- «Найди публичные Telegram-каналы про AI art»
- «Найди публикации про генерацию коротких видео в публичных каналах»
